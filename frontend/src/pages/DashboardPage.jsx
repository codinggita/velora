...
const DashboardPage = () => {
  ...
  const getLastName = () => {
    if (!user || !user.fullName) return 'Henderson';
  ...
  const navItems = [
    { name: 'Dashboard', icon: <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>, active: true },
    { name: 'Transactions', icon: <><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></>, active: false },
    { name: 'Family', icon: <><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></>, active: false },
    { name: 'Budget', icon: <><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></>, active: false },
    { name: 'Goals', icon: <><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></>, active: false },
    { name: 'Subscriptions', icon: <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon>, active: false },
    ...
    <div className="flex h-screen w-screen bg-[#FDFCF8] font-sans text-gray-800 overflow-hidden" style={{ fontFamily: 'var(--font-heading)' }}>
      ...
      <aside className="w-[260px] bg-white/80 border-r border-gray-100 flex flex-col pt-7 pb-6 px-4 shrink-0 h-full overflow-y-auto">
        ...
        <div className="flex items-center gap-2 px-3 mb-10 cursor-pointer" onClick={() => navigate('/')}>
          ...
          <span className="text-[#127475] font-semibold text-[20px] tracking-tight">Velora</span>
        </div>

        {/* Vault Title */}
        <div className="px-3 mb-6">
          <h2 className="text-[#127475] font-bold text-[18px]">The {getLastName()} Vault</h2>
          <span className="text-[11px] text-gray-400 font-medium tracking-wide">Premium Tier</span>
        </div>
        ...
        <nav className="flex flex-col gap-1.5 mb-auto flex-1 px-1">
          {navItems.map((item, index) => (
            <button
              key={index}
              className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-[13.5px] font-medium transition-all ${
                item.active 
                  ? 'bg-[#E5F5F4] text-[#127475]' 
        ...
        <div className="mt-8 px-1">
          ...
          <button className="w-full text-left py-2 px-3 text-[13px] text-gray-500 font-medium flex items-center gap-3 hover:bg-gray-50 rounded-xl transition-colors">
